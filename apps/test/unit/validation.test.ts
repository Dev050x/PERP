...
describe("authSchema", () => {
  test("valid username and password", () => {
    expect(
      authSchema.safeParse({ username: "alice123", password: "secure99" })
        .success,
    ).toBe(true);
  });
  ...
  test("rejects username shorter than 5 characters", () => {
    expect(
      authSchema.safeParse({ username: "ab", password: "secure99" }).success,
    ).toBe(false);
  });

  test("rejects password shorter than 5 characters", () => {
    expect(
      authSchema.safeParse({ username: "alice123", password: "ab" }).success,
    ).toBe(false);
  });
...
describe("deleteOrderSchema", () => {
  test("valid orderId", () => {
    expect(deleteOrderSchema.safeParse({ orderId: "uuid-123" }).success).toBe(
      true,
    );
  });
});
