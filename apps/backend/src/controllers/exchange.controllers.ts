...
const ENGINE_TIMEOUT_MS = parseInt(
  process.env.ENGINE_TIMEOUT_MS || "10000",
  10,
);
...
export const createOrder = async (
  ...
  const response = await waitForEngineResponse(
    correlationID,
    ENGINE_TIMEOUT_MS,
  );
  ...
  res.status(200).json({
    ...
    data: {
      ...
      position:
        Object.keys(response.data.position ?? {}).length > 0
...
export const getPostiion = async (req: Request, res: Response) => {
  ...
  if (response.error) {
    ...
    if (
      errStr.includes("user does not deposit") ||
      errStr.includes("deposit any asset") ||
      errStr.includes("User does not have any positions")
    ) {
      res.status(200).json({
        success: true,
    ...
    const dbCandles = await prisma.candle.findMany({
      where: {
        ...
        timestamp: {
          gte: startTime
            ? new Date(
                Number(startTime) * (Number(startTime) < 1e11 ? 1000 : 1),
              )
            : undefined,
          lte: endTime
            ? new Date(Number(endTime) * (Number(endTime) < 1e11 ? 1000 : 1))
            : undefined,
        },
      },
      orderBy: {
