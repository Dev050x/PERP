...
export function CreateOrder(data: CreateOrderData) {
  ...
  if (data.type === "market") {
    data.price = toString(
      orderbookManager.getMarketPrice(
        data.userId,
        data.slippage!,
        data.side,
        data.market,
  ...
  if (
    !userManager.hasEnoughBalance(
      userOrder.userId,
      toBigInt(userOrder.margin, PRECISION),
      "USDC",
    )
  ) {
    throw new Error("Insufficient Balance");
  }
  userManager.lockUserBalance(
    userOrder.userId,
    "USDC",
    toBigInt(userOrder.margin, PRECISION),
  );
  ...
  const status: OrderStatus = resolveStatus(
    filledQty,
    toBigInt(data.qty, PRECISION),
  );
  const fills =
    status === "open" ? [] : orderbookManager.getUserFillsByOrderId(orderId)!;
  let position = userManager.getUserPositionByMarket(
    userOrder.userId,
    userOrder.market,
  );
