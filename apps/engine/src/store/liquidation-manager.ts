...
export class LiquidationManager {
  ...
  public initializeLiquidation() {
    ...
    for (const asset of supported_markets) {
      this.liquidationLongs.set(
        asset,
        latest?.liquidationLongs.get(asset) ?? new BTree(),
      );
      this.liquidationShorts.set(
        asset,
        latest?.liquidationShorts.get(asset) ?? new BTree(),
      );
    }
  }

  public createLiquidation(
    price: bigint,
    userId: string,
    side: "LONG" | "SHORT",
    market: string,
    ...
    if (side === "LONG") {
      this.liquidationLongs.get(market)?.get(price)
        ? this.liquidationLongs.get(market)?.get(price)?.add(userId)
        : this.liquidationLongs.get(market)?.set(price, new Set([userId]));
    } else {
      this.liquidationShorts.get(market)?.get(price)
        ? this.liquidationShorts.get(market)?.get(price)?.add(userId)
        : this.liquidationShorts.get(market)?.set(price, new Set([userId]));
    }
  }

  public updateLiquidation(
    price: bigint,
    prevPrice: bigint,
    userId: string,
    side: "LONG" | "SHORT",
  ...
  public deleteLiquidation(
    ...
    if (side === "LONG") {
      ...
      this.liquidationLongs.get(market)?.get(price)?.size === 0
        ? this.liquidationShorts.get(market)?.delete(price)
        : null;
    } else {
      this.liquidationShorts.get(market)?.get(price)?.delete(userId);
      this.liquidationShorts.get(market)?.get(price)?.size === 0
        ? this.liquidationShorts.get(market)?.delete(price)
        : null;
    }
  }
  ...
  public liquidateUser(price: bigint, market: string) {
    ...
    if (this.liquidationLongs.get(market)?.length !== 0) {
      ...
      for (const [liqPrice, userIdS] of [
        ...
        for (const userId of [...userIdS]) {
          const user_position =
            UserManager.getInstance().getUserPositions(userId);
        ...
        this.liquidationLongs.get(market)?.get(liqPrice)?.size === 0
          ? this.liquidationLongs.get(market)?.delete(liqPrice)
          : null;
      }
    }
    if (this.liquidationShorts.get(market)?.length !== 0) {
      ...
      for (const [liqPrice, userIdS] of [
        ...
        this.liquidationShorts.get(market)?.get(liqPrice)?.size === 0
          ? this.liquidationShorts.get(market)?.delete(liqPrice)
          : null;
      }
    }
  }
