...
export class UserManager {
  ...
  public closeUserPosition(position: Position, price: bigint) {
    ...
    LiquidationManager.getInstance().deleteLiquidation(
    ...
    if (exisPos) {
      ...
      if (exisPos.side === side) {
        const newAvgPrice = calculateAveragePrice(
          exisPos.averagePrice,
          exisPos.qty,
          averagePrice,
          qty,
        );
        const newLiquidPrice = calculateLiquidationPrice(
          newAvgPrice,
          exisPos.qty + qty,
          exisPos.margin + margin,
          side,
        );
        LiquidationManager.getInstance().updateLiquidation(
          newLiquidPrice,
          exisPos.liquidationPrice,
          userId,
          side,
          market,
        );
      ...
      console.log(
        `price: ${averagePrice} market: ${market} qty: ${qty} margin: ${margin} `,
      );
      ...
      LiquidationManager.getInstance().createLiquidation(
        liquidationPrice,
        userId,
        side,
        market,
  ...
  public initializeUserBalance(
    userId: string,
    amount: bigint = 10000_000_000n,
  ) {
