...
export class OrderBookManager {
  ...
  public matchOrder(data: UserOrder, orderId: string): bigint {
    return data.side === "LONG"
      ? this.matchLongOrder(data, orderId)
      : this.matchShortOrder(data, orderId);
  }
  ...
  public matchLongOrder(data: UserOrder, orderId: string): bigint {
    ...
    if (toBigInt(data.qty, PRECISION) !== qty) {
      UserManager.getInstance().createUserPosition(
        ...
        averagePrice,
        data.market,
        toBigInt(data.qty, PRECISION) - qty,
        "LONG",
        toBigInt(data.margin, PRECISION),
      );
    }
    return qty;
  }
  ...
  public matchShortOrder(data: UserOrder, orderId: string): bigint {
    ...
    if (toBigInt(data.qty, PRECISION) !== qty) {
      console.log(
        `we're create the position for user ${data.userId} at average price ${averagePrice} and qty ${toBigInt(data.qty, PRECISION) - qty} with margin ${toBigInt(data.margin, PRECISION)}`,
      );
      UserManager.getInstance().createUserPosition(
        data.userId,
        averagePrice,
  ...
  public cancelBid(order: Order) {
    const bids = OrderBookManager.getInstance().getOrderbook(
      order.market,
    )!.bids;
    ...
    if (index !== -1) {
      ...
      bestPrices.get(order.price) === 0n
        ? bestPrices.delete(order.price)
        : null;

      const userDetail = UserManager.getInstance().getUser(order.userId)!;
      userDetail.orders.get(order.orderId)!.status = "Cancel";
      const userAssetBalance = UserManager.getInstance().getUserBalances(
        order.userId,
      )!["USDC"]!;
  ...
  public cancelAsk(order: Order) {
    const asks = OrderBookManager.getInstance().getOrderbook(
      order.market,
    )!.asks;
  ...
  public updateOrderBook(data: UserOrder, remainQty: bigint, orderId: string) {
    return data.side === "LONG"
      ? this.updateBids(data, remainQty, orderId)
      : this.updateAsks(data, remainQty, orderId);
  }
